...
def fim_igual(words):
  return len([palavra for palavra in words if len(palavra) >= 3 and palavra[0] == palavra[-1]])
...
def x_antes(words):
  ...
  for word in words:
    if word.startswith('x'):
      listas[0].append(word)
    else:
      listas[1].append(word)
  return sorted(listas[0], key = lambda word: (word.find('x'), word)) + sorted(listas[1])
