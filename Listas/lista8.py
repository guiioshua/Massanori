...
def preposição(txt):
    preposições = []
    for palavra in txt:
        if palavra[0] in letrasZumbi and palavra[-1] not in letrasZumbi:
            preposições.append(palavra)
    return len(preposições)
print(preposição(txtA))
print(preposição(txtB))
...
def verbo(txt):
    ...
    verbosFp = []
    for palavra in txt:
        if len(palavra)==7 and palavra[-1] not in letrasZumbi:
            verbos.append(palavra)
            if palavra[0] not in letrasZumbi:
...
def tradutorGooglon(palavra):
    alfabeto = 'ABCDEFGHIJKLMNOPQRST'
    tradução = ''
    for letra in palavra:
        tradução += alfabeto[alfabetoGooglon.index(letra)]
    return tradução

def ordenar(txt):
    listaOrdenada = sorted(txt, key=tradutorGooglon)
...
#ExE
def palavraParaNúmero(palavra):
    c = 0
    numero = 0
    i = 0
    for letra in palavra:
        c = alfabetoGooglon.index(letra)
        numero += c*20**i
        i += 1
    return numero
