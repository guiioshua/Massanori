...
def not_bad(s):
  notIndex = s.find('not')
  badIndex = s.find('bad')
  return s[:notIndex] + 'good' + s[badIndex+3:] if badIndex > notIndex else s
...
def inicio_final(a, b):
  metade = lambda s: (s[:len(s)//2+len(s)%2], s[len(s)//2+len(s)%2:])
  a_inicio, a_final = metade(a)
  b_inicio, b_final = metade(b)
...
def conta2(n):
  return sum(str(numero).count('2') for numero in range(n))
...
def inip2(n):
  expoente = acumulador = 0
  while not str(acumulador).startswith(str(n)):
    expoente += 1
    acumulador = 2**expoente
  else: return expoente
