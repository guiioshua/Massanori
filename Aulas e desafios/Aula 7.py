...
def preposição(txt):
    preposições=[]
    for palavra in txt:
        if palavra[0] in letraZumbi and palavra[-1] not in letraZumbi:
...
def verbo(txt):
    ...
    verbosFP = []
    for palavra in txt:
        if len(palavra) == 7 and palavra[-1] not in 'zmb':
            verbos.append(palavra)
        if len(palavra) == 7 and palavra[-1] not in 'zmb' and palavra[0] not in 'zmb':
